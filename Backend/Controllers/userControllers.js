...
const forgotPassword = async (req, res, next) => {
  ...
  if (!user) {
    const error = customError(
      "could not find the user with a given email",
      404
    );
    next(error);
  }
...
const filterReqObj = (reqObj, ...allowedFields) => {
  ...
  Object.keys(reqObj).forEach((prop) => {
    if (allowedFields.includes(prop)) newObj.prop = reqObj.prop;
...
const updateMe = asyncErrorHandler(async (req, res, next) => {
  ...
  const updateUser = await User.findByIdAndUpdate(req.user._id, filterObj, {
    runValidators: true,
    new: true,
  });
